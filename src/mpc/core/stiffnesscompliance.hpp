            ...
            void SetComponentsWithSymmetry(std::set< mpc::core::TensorRank4Component<T> >& components) {
                ...
                if (S::SymmetryGroupEnum() == mpc::core::SymmetryGroupEnumeration::NONE) {
                    ...
                    for (auto c : components) {
                        ...
                        int p = mpc::utilities::GetVoigtMatrixIndex(c_index.FirstIndex(), c_index.SecondIndex());
                        int q = mpc::utilities::GetVoigtMatrixIndex(c_index.ThirdIndex(), c_index.FourthIndex());

                        mat(p,q) = c.Value();
                        mat(q,p) = c.Value(); // voigt matrix is symmetrical
                    }
                    // matrix to tensor
                    mpc::utilities::VoigtMatrixToStiffnessTensor(mat, tensor);
                }
